...
public record GuessBuildRequestService(
            ...
            .chain(cloneDir -> GitUtils
                ...
                .chain(git -> analyzeSourceTree(scmRef, git)
                    .chain(buildRequestBuilder -> findReferenceArtifactsService
                        ...
                        .chain(buildGroup -> analyzeJars(buildGroup, buildRequestBuilder)
                            .chain(_ -> buildRequestBuilder.build(GitUtils.lastCommitDate(git).memoize().indefinitely()))
                        ))
    ...
    Uni<BuildRequestAlternativesBuilder> analyzeSourceTree(FqScmRef scmRef, Git git) {
        ...
        return detectMaven(workingCopy, buildToolDetector, javaDistroDetector, oses)
    ...
    Uni<BuildRequestAlternativesBuilder> detectMaven(Path workingCopy, ToolVersionDetector<BuildTool> buildToolDetector, ToolVersionDetector<String> javaDistroDetector, Set<Os> oses) {
        return findRootDirectories(workingCopy, dir -> fileSystem.exists(dir.resolve("pom.xml").toString()))
            .chain(rootDirs -> {
                ...
                return fileSystem.exists(workingCopy.resolve("mvnw").toString())
                    .chain(exists -> {
                        if (exists) {
                            buildToolDetector.toolVersions.computeIfAbsent(BuildTool.maven_wrapper, _ -> new ConcurrentHashSet<>());
                        }
                        return Uni.createFrom().item(new BuildRequestAlternativesBuilder(rootDirs, buildToolDetector, javaDistroDetector, oses));
                    });
            });
    }
                ...
                .chain(jarFile -> extractFile(jarFile, "META-INF/MANIFEST.MF", byte[].class)
                    .chain(manifest -> {
                        ...
                        try {
                            ...
                            throw new UncheckedIOException("Could not load manifest from " + jarFile + "!META-INF/MANIFEST.MF", e);
                        }
        ...
        Uni<Void> javadocInfos = Multi.createFrom().iterable(javadocJars)
            .onItem().transformToUni(jar -> jar.map(gavtc -> gavtc.getFile())
                ...
                .chain(html -> {
                    ...
                    if (m.find()) {
                        buildInfo.javaDistroDetector.toolVersions.computeIfAbsent(JavaDistroAndVersion.UNKNOWN, k -> new ConcurrentHashSet<>())
                            .add(m.group(1));
                    }
                    return Uni.createFrom().voidItem();
                }))
...
//        })
    ...
    public static final class BuildRequestAlternativesBuilder {
        ...
        private final MavenVersionsService mavenVersionsService;
        ...
        public BuildRequestAlternativesBuilder(
            ...
            this.mavenVersionsService = mavenVersionsService;
            this.rootPaths = rootPaths;
            this.buildToolDetector = buildToolDetector;
            this.javaDistroDetector = javaDistroDetector;
        }
                ...
                if (versions.isEmpty()) {
                    if (bt.isWrapper()) {
                        ...
                        Uni<BuildToolAndVersion> item = releaseDate
                            .chain(reldate -> bt.latestVersionAsOf(reldate))
        ...
        public Uni<Set<JavaDistroAndVersion>> javaDistroAndVersion(Uni<ZonedDateTime> releaseDate) {
            ...
            return
                Uni.join().all(result).andCollectFailures().map(list -> Collections.unmodifiableSet(new TreeSet<>(list)));
        }
        ...
        String createBuildScript(BuildTool bt, BuildGroup<FqScmRef> buildGroup, Os os, Shell shell) {
        ...
        Uni<BuildRequestAlternatives> build(Uni<ZonedDateTime> releaseDate) {
            ...
            return Uni.combine()
                ...
                .onItem().transform(tuple -> {
                    ...
                    BuildToolAndVersion bt = bts.iterator().next();
                    Os os = os();
                    Shell shell = os.defaultShell();
                    String buildScript = createBuildScript(bt.buildTool(), buildGroup, os, shell);
